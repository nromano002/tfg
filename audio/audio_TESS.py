...
archivo_txt= '../../resultados/audio/SER_TESS_'+modelo+'_'+featurestr+'.txt'
archivo_h5 = '../../modelos/audio/SER_TESS_'+modelo+'_'+featurestr+'.h5'


"""Dataset TESS """
TESS = "../../inputs/TESS Toronto emotional speech set data/"
dir_list = os.listdir(TESS)
dir_list.sort()
print(dir_list) #imprime los diferentes directorios
...
labels = {'female_disgust':0,'female_happy':1,'female_sad':2,'female_neutral':3,'female_fear':4,'female_angry':5,'female_surprise':6}
TESS_df.replace({'Emotions':labels},inplace=True)
...
for i in range(2800):
    data['labels'].append(TESS_df.iloc[i,0])
    signal, sample_rate = librosa.load(TESS_df.iloc[i,1], sr=SAMPLE_RATE)
...
for train_index, test_index in skf.split(X,y):
    ...
    if modelo!='MLP':
        ...
        cm = confusion_matrix(y_test, pred, labels=[0,1,2,3,4,5,6])
