...
archivo_txt= '../../resultados/audio/SER_SAVEE_'+modelo+'_'+featurestr+'.txt'
archivo_h5 = '../../modelos/audio/SER_SAVEE_'+modelo+'_'+featurestr+'.h5'


"""Dataset SAVEE """
SAVEE = "../../inputs/savee/ALL/"

dir_list = os.listdir(SAVEE)

# parsear el nombre del archivo para obtener las emociones
emotion=[]
path = []
for i in dir_list:
    if i[-8:-6]=='_a':
...
labels = {'male_disgust':0,'male_happy':1, 'male_sad':2,'male_neutral':3,'male_fear':4,'male_angry':5,'male_surprise':6}
SAVEE_df.replace({'Emotions':labels},inplace=True)
...
for i in range(480):
    data['labels'].append(SAVEE_df.iloc[i,0])
    signal, sample_rate = librosa.load(SAVEE_df.iloc[i,1], sr=SAMPLE_RATE)
